...
| awesome-mcp-servers (punkpeye) | 2026-09-07 | skipped (owner decision) | branch kept: https://github.com/alexandrrzk-ctrl/awesome-mcp-servers/tree/add-alexcredit-catalog | the list's bot requires a Glama *server* quality badge, which needs a Glama release built from a Dockerfile; this repo is documentation-only (server code is private). Low traffic value (400+ Finance entries). Revisit only if a stdio bridge with Dockerfile is ever added |
...
| Glama — Connector | 2026-09-07 | live, ownership verified, all 7 tools graded A (TDQS) | https://glama.ai/mcp/connectors/kz.alexcredit/alex-credit-kazakhstan-microfinance-catalog | approval e-mail received 2026-09-07; ownership claim via HTTP token: https://alexcredit.kz/.well-known/glama.json published (served by nginx, file tracked in the frontend repo) — never delete, Glama re-verifies periodically; ownership verified 2026-09-07 |
| Glama — Server (GitHub repo) | 2026-09-07 | skipped | — | Server tab not offered in the add form; a documentation-only repo cannot get a Glama release/score anyway. Connector listing covers discovery |
