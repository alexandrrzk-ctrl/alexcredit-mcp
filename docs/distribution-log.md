...
| Glama | 2026-09-07 | Connector submitted for review; Server (GitHub repo) tab still to submit | expected https://glama.ai/mcp/servers/alexandrrzk-ctrl/alexcredit-mcp | Connector tab (endpoint https://alexcredit.kz/api/mcp) submitted by the owner. The Server tab with the repo URL is what produces the /mcp/servers/alexandrrzk-ctrl/alexcredit-mcp listing and the score badge required by the awesome-mcp-servers bot — submit it too, then claim |
